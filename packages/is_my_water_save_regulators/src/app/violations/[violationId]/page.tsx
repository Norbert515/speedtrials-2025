...
export default function ViolationDetailPage() {
  ...
  const handleTabChange = (value: string) => {
    ...
    router.replace(`/violations/${violationId}?${params.toString()}`, {
      scroll: false,
    });
  };
  ...
  const fetchViolationDetails = async () => {
    try {
      ...
      // First, find which PWSID this violation belongs to
      const { data: violationLookup, error: lookupError } = await supabase
        .from("violations_enforcement")
        .select("pwsid")
        .eq("violation_id", violationId)
        .single();

      if (lookupError || !violationLookup) {
      ...
      const { data: violations, error: violationsError } = await supabase.rpc(
        ...
        { system_pwsid: violationLookup.pwsid }
      );
      ...
      const violationData = (violations as ViolationDetail[])?.find(
        (v: ViolationDetail) => v.violation_id === violationId
      );
      ...
      const completeViolationData = {
        ...
        federal_mcl: "", // Default
        state_mcl: 0, // Default
        calculated_pub_notif_tier: violationData.public_notification_tier, // Default
      };

      setViolation(completeViolationData);
    } catch (error) {
