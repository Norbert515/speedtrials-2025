...
-- This fixes the type mismatch error causing 400 Bad Request
...
-- Function to get systems sorted by health status (worst first) with correct types
...
-- Function to get violations with explanations, with correct types
...
-- Add comments
COMMENT ON FUNCTION get_systems_sorted IS 'Get water systems sorted by health status (worst first) with pagination - fixed types';
COMMENT ON FUNCTION get_violations_with_explanations IS 'Get violations for a system with AI explanations, fallback to basic data if no explanations available - fixed types'; 